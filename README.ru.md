...
Изменяемые браузерные селекторы и тексты подтверждений HeadHunter находятся в
`config/browser/headhunter_apply.json`. Docker Compose подключает этот профиль к API и браузерному
воркеру в режиме только для чтения. Если HeadHunter изменит форму отклика, достаточно обновить
соответствующий список селекторов и перезапустить `api` и `browser-worker` — менять Python-код не
требуется. При запуске адаптер проверяет профиль и безопасно останавливает сценарий, если файл
повреждён или неполон.
