...
class CheckStepParameters(StrictWorkflowStepModel):
    value_key: str = Field(min_length=1, max_length=200)


class CheckWorkflowStep(StrictWorkflowStepModel):
    action_type: Literal[WorkflowStepType.CHECK] = WorkflowStepType.CHECK
    selector_candidates: list[WorkflowSelectorCandidate] = Field(min_length=1, max_length=20)
    parameters: CheckStepParameters
