...
class TestStratifiedSampling:
    ...
    def test_sample_size_respects_limit(self):
        ...
        candidates = [
            self.make_candidate(f"v{i}", current_rank=i, ltr_rank=i) for i in range(1, 200)
        ]
    ...
    def test_all_strata_represented(self):
        ...
        result = _stratified_sample(candidates, limit=100)
        # a candidate can belong to several strata; sampling_reason is only the highest one
        strata_found = {stratum for c in result for stratum in c.strata}
    ...
    def test_deterministic_sampling(self):
        ...
        candidates = [
            self.make_candidate(f"v{i}", current_rank=i, ltr_rank=i) for i in range(1, 150)
        ]
    ...
    def test_overlapping_strata_priority(self):
        ...
        # v1 is top-1 by the current ranking but only 50th by LTR, so it is NOT an ltr_top member
        assert "ltr_top" not in v1.strata
    ...
    def test_fewer_candidates_than_limit(self):
        ...
        candidates = [
            self.make_candidate(f"v{i}", current_rank=i, ltr_rank=i) for i in range(1, 50)
        ]
        result = _stratified_sample(candidates, limit=100)
    ...
    def test_middle_rank_adapts_to_candidate_count(self):
        ...
        # Small dataset
        candidates = [
            self.make_candidate(f"v{i}", current_rank=i, ltr_rank=i) for i in range(1, 60)
        ]
        result = _stratified_sample(candidates, limit=100)
    ...
    def test_strata_counts_respect_quotas(self):
        ...
        candidates = [
            self.make_candidate(f"v{i}", current_rank=i, ltr_rank=i) for i in range(1, 500)
        ]
        result = _stratified_sample(candidates, limit=100)
    ...
    def test_sorting_by_priority_then_rank(self):
        ...
        for c in result:
            priority = (
                STRATA_PRIORITY.index(c.sampling_reason)
                if c.sampling_reason in STRATA_PRIORITY
                else 99
            )
            assert priority >= prev_priority, f"Priority decreased: {prev_priority} -> {priority}"
            if priority == prev_priority:
                assert c.current_rank >= prev_rank, (
                    f"Rank not sorted within priority: {prev_rank} -> {c.current_rank}"
                )
